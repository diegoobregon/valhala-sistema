...
[System.IO.File]::WriteAllText("$root\src\app\services\api.service.ts", @'
...
[System.IO.File]::WriteAllText("$root\src\app\trans.component.ts", @'
...
$p = "$root\src\app\app.ts"
$c = [System.IO.File]::ReadAllText($p)
$c = $c.Replace("import { ApiService } from './services/api.service';", "import { ApiService } from './services/api.service';`nimport { TransComponent } from './trans.component';")
$c = $c.Replace("imports: [CommonModule, FormsModule],", "imports: [CommonModule, FormsModule, TransComponent],")
$c = $c.Replace("Docs Legales</a>", "Docs Legales</a>`n        <a *ngIf=""puede(['ADMIN','MECANICO'])"" [class.on]=""view==='trans'"" (click)=""view='trans'"">Transacciones (Check-Out / Check-In)</a>")
$c = $c.Replace("<footer>VALHALA", "<app-trans *ngIf=""view==='trans'""></app-trans>`n      <footer>VALHALA")
$c = $c.Replace("return m[this.view] || 'VALHALA S.A.C.';", "return m[this.view] || 'Transacciones | Check-Out / Check-In (RF-05, RF-06)';")
[System.IO.File]::WriteAllText($p, $c, $utf8)
Write-Host "MODULO TRANSACCIONES INYECTADO! Ctrl+F5"