...
public interface ReservaGanttRepository extends JpaRepository<ReservaGantt, Integer> {
    ...
    long contarColisiones(@Param("idItem") Integer idItem, @Param("inicio") LocalDate inicio, @Param("fin") LocalDate fin);
    List<ReservaGantt> findByEstadoReservaInOrderByFechaInicioReserva(List<String> estados);
    @Query("SELECT r FROM ReservaGantt r WHERE r.itemContrato.contrato.empresa.idEmpresa = :idEmpresa")
    List<ReservaGantt> findByIdEmpresa(@Param("idEmpresa") Integer idEmpresa);
}